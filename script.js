...
const quizArray = [
  ...
  {
    id: "3",
    question: "現在行っている情報漏洩対策はありますか？",
    options: [
      { label: "２段階認証の導入", value: 0 },
      { label: "EDRの導入", value: 1 },
      { label: "セキュリティポリシーの策定", value: 2 },
    ],
    multi: true,
    // correct: "いいえ",
  },
...
nextBtn.addEventListener(
  ...
  (displayNext = () => {
    let selected_arr = [];
    quizArray[questionCount].options.forEach((option) => {
      if (current_select_vals.includes(option)) {
      }
    });
    // if (quizArray[questionCount].options.indexOf(current_select_val) >= 0) {
    //   send_object[questionCount + 1] = []
    //   // send_array.push("1");
    // } else {
    //   send_array.push("");
    // }
...
function checker(userOption) {
  ...
  current_select_vals.push(userSolution);
